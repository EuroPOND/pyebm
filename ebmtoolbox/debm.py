...
def fit(DataIn, MethodOptions=False, VerboseOptions=False, Factors=['Age', 'Sex', 'ICV'], Labels=['CN', 'MCI', 'AD'],
        DataTest=[]):
    ...
    DMO.JointFit = 0
    DMO.MixtureModel = 'vv2'
    DMO.Bootstrap = 0
    DMO.PatientStaging = ['exp', 'p']
    DVO.Distributions = 0
    DVO.Ordering = 0
    DVO.PlotOrder = 0
    DVO.WriteBootstrapData = 0
    DVO.PatientStaging = 0
    ...
    idx_CN = pdData_all['Diagnosis'].values == 1
    idx_AD = pdData_all['Diagnosis'].values == 3
    idx_MCI = pdData_all['Diagnosis'].values == 2
    ...
    ptid_AD_raw = pdData_all.loc[idx_AD, 'PTID']
    ptid_CN_raw = pdData_all.loc[idx_CN, 'PTID']
    ptid_MCI_raw = pdData_all.loc[idx_MCI, 'PTID']
    ...
    ModelOutput.BiomarkerList = BiomarkersList
    ModelOutput.MeanCentralOrdering = pi0_mean
    ModelOutput.EventCenters = event_centers_all
    ModelOutput.CentralOrderings = pi0_all
